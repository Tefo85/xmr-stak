...
size_t InitOpenCLGpu(cl_context opencl_ctx, GpuContext* ctx, const char* source_code)
{
	...
	snprintf(options, sizeof(options), 
		"-DITERATIONS=%d -DMASK=%d -DWORKSIZE=%llu -DSTRIDED_INDEX=%d -DCOMP_MODE=%d",
		hasIterations, threadMemMask, int_port(ctx->workSize), ctx->stridedIndex ? 1 : 0, ctx->compMode ? 1 : 0);
